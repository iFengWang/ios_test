...
@implementation XSYWorkFlowViewModel

@end
@implementation XSYWorkFlowViewTarget {
    NSString *_apiKey;
    NSString *_recordId;
}

+ (instancetype)targetWithObjectApiKey:(NSString *)apiKey recordId:(NSString *)recordId {
    return [[XSYWorkFlowViewTarget alloc] initWithObjectApiKey:apiKey recordId:recordId];
}
...
- (void)request:(void (^)(XSYResult *_Nonnull))completeBlock {
    [XSYProvider.defaultProvider requestWithTarget:self
                                       transformer:^id(NSDictionary<NSString *, id> *json) {
                                           return [XSYWorkFlowViewModel mj_objectWithKeyValues:json[@"data"]];
                                       }
                                        completion:^(XSYResult<XSYWorkFlowViewModel *> *result) {
