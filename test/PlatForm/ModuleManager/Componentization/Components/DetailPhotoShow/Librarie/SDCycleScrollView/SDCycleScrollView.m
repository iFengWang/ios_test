...
@property (nonatomic,strong)  UIView * custonPageBack;
@property (nonatomic,strong)  UIView * custonPage;
@property (nonatomic,strong)  UILabel * custonPageText;
@end
...
- (void)setupMainView
{
    ...
    [self addSubview:self.custonPageBack];
    [self bringSubviewToFront:self.custonPageBack];
    [self addSubview:self.custonPage];
    [self bringSubviewToFront:self.custonPage];
    [self.custonPage addSubview:self.custonPageText];
...
- (void)setImagePathsGroup:(NSArray *)imagePathsGroup
{
    ...
    [self setupPageControl];
    self.custonPageText.text = [NSString stringWithFormat:@"%d/%d",1,self.imagePathsGroup.count];
...
- (void)layoutSubviews
{
    ...
    self.custonPage.frame = CGRectMake(self.width-60, self.height-45, 44, 25);
    self.custonPageBack.frame = CGRectMake(self.width-60, self.height-45, 44, 25);
    self.custonPageText.frame = CGRectMake(0, 0, 44, 25);
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.delegate respondsToSelector:@selector(selectCycleScrollView:didSelectItemAtIndex:)]) {
        [self.delegate selectCycleScrollView:cell.imageView didSelectItemAtIndex:[self pageControlIndexWithCurrentCellIndex:indexPath.item]];
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    self.custonPageText.text = [NSString stringWithFormat:@"%d/%d",indexOnPageControl+1,self.imagePathsGroup.count];
...
- (UIView*)custonPage{
    if (!_custonPage) {
        _custonPage = [UIView new];
        _custonPage.backgroundColor = [UIColor clearColor];
        _custonPage.layer.masksToBounds = YES;
        _custonPage.layer.cornerRadius = 5;
    }
    return _custonPage;
}
- (UIView*)custonPageBack{
    if (!_custonPageBack) {
        _custonPageBack = [UIView new];
        _custonPageBack.backgroundColor = [UIColor colorWithRed:(200) / 255.0f green:(200) / 255.0f blue:(200) / 255.0f alpha:0.8];
        _custonPageBack.layer.masksToBounds = YES;
        _custonPageBack.layer.cornerRadius = 5;
    }
    return _custonPageBack;
}
- (UILabel*)custonPageText{
    if (!_custonPageText) {
        _custonPageText = [ViewCreatorHelper createLabelWithTitle:@"" font:[UIFont systemFontOfSize:15.0f] frame:CGRectZero textColor:[UIColor whiteColor] textAlignment:NSTextAlignmentCenter];
    }
    return _custonPageText;
}
@end
